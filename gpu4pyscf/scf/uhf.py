...
from gpu4pyscf.scf.hf import (
    eigh, damping, level_shift, tag_array, asarray, return_cupy_array,
    BACKEND_NAME,
)
...
def get_fock(mf, h1e=None, s1e=None, vhf=None, dm=None, cycle=-1, diis=None,
    ...
    if damp_factor is not None and 0 <= cycle < diis_start_cycle-1 and fock_last is not None:
        if isinstance(damp_factor, (tuple, list, np.ndarray)):
            ...
            dampa = dampb = damp_factor
        f = np.asarray((damping(f[0], fock_last[0], dampa),
...
def energy_elec(mf, dm=None, h1e=None, vhf=None):
    ...
    e1 = float(np.asarray(e1).real)
    e_coul = float(np.asarray(e_coul).real)
    e_elec = e1 + e_coul
...
class UHF(hf.SCF):
    ...
    get_init_guess           = return_cupy_array(uhf_cpu.UHF.get_init_guess)
    ...
    def eig(self, fock, s, overwrite=False, x=None):
        ...
        nao, nmo = c_a.shape
        c = np.empty((2, nmo, nao), dtype=c_a.dtype).transpose(0,2,1)
        c[0] = c_a
        c[1] = c_b
        return np.stack((e_a,e_b)), c
    ...
    def get_veff(self, mol=None, dm=None, dm_last=0, vhf_last=0, hermi=1):
        ...
        vhf = np.asarray(vhf) * -1
        vhf += np.asarray(vj)
        if vhf_last is not None:
            vhf = vhf + np.asarray(asarray(vhf_last))
        return vhf
    ...
    def Gradients(self):
        from gpu4pyscf.grad import UHF as UHFGrad
        return UHFGrad(self)
